...
def run_scenario(input_objects, input_links, input_global, scenario_name = "instance_demo1_N10", rum_lim_minutes = 3, disable_costly_constraints = False, force_1_to_catch_a_bus = False, show_fig = False, return_resolution_stats = False):
    ...
    if return_resolution_stats == True:
        a_temp = math.floor(md.ObjVal*100)/100
        b_temp = math.floor(md.ObjBound*100)/100
        c_temp = math.floor(md.Runtime*100)/100
        d_temp = math.floor(md.SolCount*100)/100
        return [str(a_temp) + " " + str(b_temp) + " " + str(c_temp) + " " + str(d_temp)]
...
def return_component_scores(model):
    ...
    input_links             = model._input_links
